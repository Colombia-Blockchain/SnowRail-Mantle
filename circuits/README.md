...
Zero-knowledge circuits for the Mantle AI-Powered Treasury.
...
**Contract:** TBD (Pending deployment on Mantle Sepolia)

Proves valid withdrawal from mixer WITHOUT revealing which deposit.

**Use Case:** Alice deposits 0.1 MNT. Later, Bob withdraws 0.1 MNT proving he knows a valid commitment. Observers CANNOT link Alice to Bob.
...
curl -X POST http://localhost:3001/api/mixer/generate-note
...
|---------|----------|---------|
| mixer | ZKMixer | TBD (Pending deployment on Mantle Sepolia) |
| price_condition | (Agent-only) | N/A - used off-chain |

**Note:** ZKMixer will be deployed on Mantle Sepolia (Chain ID: 5003). Check [MantleScan](https://sepolia.mantlescan.xyz) for the latest deployment.
